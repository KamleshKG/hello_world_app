...
class DiagramApp:
    ...
    def _show_text_diagram(self):
        ...
        # Example buttons
        ex_frame = tk.Frame(dlg, bg=SURFACE2); ex_frame.pack(fill="x")
        tk.Label(ex_frame, text="Examples:", bg=SURFACE2, fg=TEXT_MUTED,
...
Dev Server --> Developer [label: notify]""",
            ...
            "Auth Flow": """# User Authentication Flow
Actor: User
UI: Browser
API: Auth Service [color: purple]
Database: User DB [color: teal]
System: JWT Service [color: orange]
System: Email Service [color: gray]

User -> Browser [label: enter credentials]
Browser -> Auth Service [label: POST /login]
Auth Service -> User DB [label: lookup user]
User DB --> Auth Service [label: user record]
Auth Service -> JWT Service [label: sign token]
JWT Service --> Auth Service [label: JWT token]
Auth Service --> Browser [label: 200 OK + token]
Browser --> User [label: redirect dashboard]
Auth Service -> Email Service [label: log event]""",
...
Queue: Kafka [color: yellow]
Database: Orders DB [color: green]
Database: Payments DB [color: green]

Client -> API Gateway [label: HTTP]
API Gateway -> Order Service
API Gateway -> Payment Service
Order Service -> Orders DB
Order Service -> Kafka [label: order.placed]
Payment Service -> Payments DB
Payment Service -> Kafka [label: payment.done]
Kafka -> Inventory Service [label: reserve stock]
Kafka -> Notification Service [label: send email]
Inventory Service <-> Order Service [label: stock check]""",

            "DevSecOps": """# DevSecOps Pipeline
Actor: Developer
System: Git [color: gray]
System: Jenkins [color: orange]
System: SonarQube [color: blue]
System: Trivy Scanner [color: red]
System: Nexus Registry [color: purple]
System: Kubernetes Dev [color: teal]
System: Kubernetes Prod [color: green]
System: Prometheus [color: orange]

Developer -> Git [label: git push]
Git -> Jenkins [label: webhook]
Jenkins -> SonarQube [label: code quality]
Jenkins -> Trivy Scanner [label: image scan]
Jenkins -> Nexus Registry [label: push image]
Nexus Registry -> Kubernetes Dev [label: deploy]
Kubernetes Dev --> Jenkins [label: health check]
Jenkins -> Kubernetes Prod [label: promote]
Kubernetes Dev -> Prometheus [label: metrics]
Kubernetes Prod -> Prometheus [label: metrics]""",
        }
        for name, txt in EXAMPLES_TXT.items():
            def load_ex(t=txt):
                editor.delete("1.0", tk.END)
                editor.insert("1.0", t)
                on_change()
            tk.Button(ex_frame, text=name, bg=SURFACE2, fg=ACCENT2,
