...
class DiagramApp:
    ...
    def __init__(self, root):
        ...
        self.focus_enabled  = tk.BooleanVar(value=True)   # highlight on select
        self._focus_ids     = set()   # box ids in focus (selected + neighbours)
        self._focus_arrow_ids = set() # arrow ids connected to focused boxes
    ...
    def _build_ui(self):
        ...
        for text, var, tip in [
            ...
            ("◎ Focus",  self.focus_enabled,  "Highlight connected nodes on select"),
        ]:
    ...
    def _build_info(self, parent):
        ...
        for key,desc in [("Ctrl+S","Save"),("Ctrl+O","Open"),("Ctrl+Z","Undo"),
                         ...
                         ("Del","Delete"),("Esc","Deselect / unfocus"),
                         ("S","Toggle snap"),("O","Toggle ortho"),
                         ("F","Toggle focus mode"),
    ...
    def _bind_events(self):
        ...
        self.root.bind("<f>",        lambda e:(self.focus_enabled.set(not self.focus_enabled.get()),self._draw_all()))
        ...
        if self.mode=="select":
            ...
            if hit:
                ...
                if not shift:
                    ...
                    self._focus_ids.clear(); self._focus_arrow_ids.clear()
                # Start rubber-band
    ...
    def _compute_focus(self):
        """Compute which box ids and arrow ids are in focus.
        Focus = selected_items + their direct neighbours (1-hop).
        If nothing selected or focus disabled, focus_ids is empty (= show all equally)."""
        self._focus_ids.clear()
        self._focus_arrow_ids.clear()
        if not self.focus_enabled.get(): return
        if not self.selected_items: return

        # Start with selected boxes
        self._focus_ids.update(self.selected_items)

        # Find all arrows connected to selected boxes and their neighbours
        for a in self.arrows:
            if a.src_id in self.selected_items or a.dst_id in self.selected_items:
                self._focus_arrow_ids.add(a.id)
                # Add the other end as a neighbour
                self._focus_ids.add(a.src_id)
                self._focus_ids.add(a.dst_id)

    def _draw_all(self):
        self._compute_focus()
        c=self.canvas; c.delete("all")
        self._draw_grid()
        # Draw dimmed arrows first, then focused arrows on top
        ...
        if isinstance(self.selected,Box):
            ...
            # Focus ring — bright glow around selected box
            if self._focus_ids and self.selected.id in self.selected_items:
                x1,y1=self._to_canvas(self.selected.x,self.selected.y)
                x2,y2=self._to_canvas(self.selected.x+self.selected.w,
                                       self.selected.y+self.selected.h)
                self.canvas.create_rectangle(x1-4,y1-4,x2+4,y2+4,
                    outline=SEL_COL,width=2.5,fill="",dash=())
        # Rubber-band rect
        if self._rband_rect:
    ...
    def _draw_box(self,box):
        ...
        # Focus dimming
        in_focus  = (not self._focus_ids) or (box.id in self._focus_ids)
        is_centre = box.id in self.selected_items  # directly selected
        is_neighbour = in_focus and not is_centre and bool(self._focus_ids)

        if not in_focus:
            # Draw dimmed version with stipple
            c.create_rectangle(x1,y1,x2,y2,fill=fill,outline=border,
                               width=0.5,stipple="gray50")
            c.create_text((x1+x2)/2,(y1+y2)/2,text=box.label,
                          fill="#888aaa",
                          font=("Consolas",max(6,int(10*self.zoom))),
        ...
        # Neighbour gets a subtle coloured ring
        if is_neighbour:
            c.create_rectangle(x1-3,y1-3,x2+3,y2+3,fill="",
                               outline="#ffcc44",width=1,dash=(4,3))
    ...
    def _draw_arrow(self,arrow):
        ...
        # Focus dimming for arrows
        in_focus_arrow  = (not self._focus_ids) or (arrow.id in self._focus_arrow_ids)
        is_sel=(self.selected is arrow)

        if not in_focus_arrow and self._focus_ids:
            # Draw very dim arrow
            dash_dim=(6,4) if getattr(arrow,"line_style","solid")=="dashed" else ()
            c.create_line(cx1,cy1,cx2,cy2,fill="#4a4a7a",width=1.5,dash=dash_dim)
            return

        # Focused arrow — bright gold if directly connected to selected, else normal
        is_direct = (arrow.src_id in self.selected_items or
                     arrow.dst_id in self.selected_items)
        if is_direct and self._focus_ids and not is_sel:
            color="#ffcc44"; lw=2.5
        elif is_sel:
            color=SEL_COL; lw=2.5
        else:
    ...
    def _draw_floattext(self,ft):
        ...
        # Dim float text when focus is active
        col=SEL_COL if is_sel else (ACCENT2 if not self._focus_ids else "#666688")
    ...
    def _update_sel_info(self):
        ...
        if n>1:
            # Count total arrows in focus
            conn = len(self._focus_arrow_ids) if self._focus_ids else 0
            self.sel_info.config(text=f"{n} selected\n{len(self._focus_ids)} in focus\n{conn} connections")
...
SIEM Splunk -> API Gateway [label: collect logs]""",
        ...
        def save_dsl():
            path = filedialog.asksaveasfilename(
                ...
                title="Save DSL")
            if path:
                with open(path,"w",encoding="utf-8") as f: f.write(editor.get("1.0",tk.END))
        ...
        def load_dsl():
            ...
            if path:
                ...
                editor.insert("1.0", open(path, encoding="utf-8", errors="replace").read())
                on_change()
